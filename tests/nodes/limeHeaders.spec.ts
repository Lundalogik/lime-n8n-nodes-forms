...
function makeContext(
    ...
    return {
        ...
        getWorkflow: jest
            .fn()
            .mockReturnValue({ id: workflowId, name: workflowName }),
...
describe('buildLimeHeaders', () => {
    ...
    it('URL-encodes workflow and node names containing special characters', () => {
        ...
        expect(headers['X-N8N-Workflow-Name']).toBe(
            'Sync%20ACME%20Corp%20%2F%20jens%40customer.com'
        );
        expect(headers['X-N8N-Node-Name']).toBe(
            'Step%20%231%20%E2%80%94%20fetch'
        );
    });
    ...
    it('truncates free-form values to 200 codepoints and stays decodable', () => {
        ...
        const headers = buildLimeHeaders(
            makeContext({ workflowName: emojiName, nodeName: emojiName })
        );
    ...
    it('drops headers whose source value is empty', () => {
        const headers = buildLimeHeaders(
            makeContext({ workflowId: '', workflowName: '' })
        );

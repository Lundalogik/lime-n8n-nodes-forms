...
function makeContext(overrides?: { onError?: string }): IExecuteFunctions {
	return {
		...
		getCredentials: jest.fn().mockResolvedValue({ url: 'https://forms.example.com' }),
...
describe('getReceipt', () => {
	it('fetches a submission receipt PDF successfully', async () => {
		...
		const result = await getReceipt(ctx, 'submission-slug-123', ReceiptType.SUBMISSION);
	...
	it('returns an error envelope when the API call fails on Forms 3.x', async () => {
		...
		const result = await getReceipt(ctx, 'nonexistent-slug', ReceiptType.SUBMISSION);

		expect(result.success).toBe(false);
		if (!result.success) {
			expect(result.data.error.message).toContain('Receipt not available');
		}
	});
	...
	it('returns version requirement message when endpoint not found on Forms 2.x', async () => {
		...
		const ctx = makeContext({ onError: 'continueRegularOutput' });
		const result = await getReceipt(ctx, 'some-slug', ReceiptType.SUBMISSION);
